 ...
 */
// Originally under The Unlicense (http://unlicense.org), but has been modified since.
...
export module util.fpconvert;

export namespace FpConvert {
...
struct raw_float_type_info {
    ...
    static constexpr auto abs(raw_type v) {
        return raw_type(v & (sign - 1));
    }
    static constexpr bool is_nan(raw_type v) {
        return abs(v) > inf;
    }
    static constexpr bool is_inf(raw_type v) {
        return abs(v) == inf;
    }
    static constexpr bool is_zero(raw_type v) {
        return abs(v) == 0;
    }
};
...
template<
    typename T,
    int SIG_BITS = std::numeric_limits<T>::digits - 1,
    int EXP_BITS = sizeof(T) * CHAR_BIT - SIG_BITS - 1>
struct float_type_info : raw_float_type_info<native_float_bits_t<T>, SIG_BITS, EXP_BITS> {
    using flt_type = T;
...
struct raw_float_encoder {
    ...
    static auto encode(F value) {
        ...
        if constexpr (!do_rounding) {  // fix exp bias
            ...
            value *= bit_cast<F>(bias_mul);
        }
        auto bits = bit_cast<raw_type>(value);
        ...
        if constexpr (do_rounding) {
            ...
            auto norm = bit_cast<F>(bits);
            auto subn = norm;
            subn *= bit_cast<F>(sub_rnd);  // round subnormals
            subn *= bit_cast<F>(sub_mul);  // correct subnormal exp
            norm *= bit_cast<F>(bias_mul);  // fix exp bias
            bits = bit_cast<raw_type>(norm);
