...
bool Program::ProgramLoader::getWord(uint32_t& res) {
    if (idx + 4 > length)
        return false;
...
RayTraceSubstage& Program::getSubstage(RtStageKind stage, unsigned index) {
...
void Program::launchSubstage(RtStageKind stage, std::vector<Frame*>& frame_stack) {
...
void Program::completeSubstage(RtStageKind stage, Frame& launched_from) {
    RayTraceSubstage& rt_stage = getSubstage(stage, launched_from.getRtIndex());
    rt_stage.cleanUp(launched_from);
}

void Program::parse(std::string file_path, uint8_t* buffer, int length) noexcept(false) {
...
void Program::initRaytrace(RayTraceSubstage& stage, spv::ExecutionModel expected, ValueMap& extra_inputs, bool unused) {
...
void Program::checkInputs(ValueMap& provided, bool unused) noexcept(false) {
