...
protected:
    ...
    public:
        inline LineHandler(const std::string* start_line, std::istream* file)
        ...
        inline std::tuple<const std::string*, unsigned> update() {
            return {pLine, idx};
        }
    ...
    inline bool isNested(const Value& val) const {
    ...
    inline void newline(std::stringstream& out, bool spaces, unsigned indents) const {
        out << '\n';
...
public:
    ...
    inline void setTemplate(bool print_template) {
        templatize = print_template;
    }
    inline void setIndentSize(unsigned size_in_spaces) {
