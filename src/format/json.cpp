...
[[nodiscard]] Value* Json::parseValue(LineHandler& handler) {
...
std::string Json::parseString(LineHandler& handler) const {
...
std::optional<char> Json::skipWhitespace(LineHandler& handler) const {
...
void Json::printKey(std::stringstream& out, const std::string& key) const {
...
void Json::verifyBlank(LineHandler& handler) noexcept(false) {
...
void Json::printFile(std::stringstream& out, const ValueMap& vars) {
    out << "{";
    bool first = true;
