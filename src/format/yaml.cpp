...
std::optional<char> Yaml::skipWhitespace(LineHandler& handler, bool break_at_newline) const {
    while (true) {
...
Yaml::parseAgg(LineHandler& handler, unsigned indent, bool list, std::string seen_name) {
...
[[nodiscard]] Value* Yaml::parseInlineAgg(LineHandler& handler, bool list) {
...
std::string Yaml::parseString(LineHandler& handler) const {
...
void Yaml::verifyBlank(LineHandler& handler, bool break_at_newline) {
    while (true) {
...
void Yaml::printKey(std::stringstream& out, const std::string& name) const {
...
unsigned Yaml::countIndent(LineHandler& handler, bool break_at_newline) const {
    unsigned indent = 0;
    while (true) {
        auto cc = handler.peek();
        if (!cc.has_value())
...
[[nodiscard]] std::tuple<Value*, bool> Yaml::parseValue(LineHandler& handler, unsigned min_indent) {
...
void Yaml::printFile(std::stringstream& out, const ValueMap& vars) {
    bool first = true;
