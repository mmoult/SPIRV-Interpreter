 ...
 */
// Edited version of https://www.shadertoy.com/view/MdXyzX
// afl_ext 2017-2024
// MIT License
#version 460
...
layout(location = 0) in vec2 iMouse;
layout(location = 1) flat in ivec2 iResolution;
...
float getwaves(vec2 position, int iterations) {
  float wavePhaseShift = length(position) * 0.1; // this avoids every octave having exactly the same phase everywhere
...
vec3 getRay(vec2 fragCoord) {
  ...
  return createRotationMatrixAxisAngle(vec3(0.0, -1.0, 0.0), 3.0 * ((NormalizedMouse.x + 0.5) * 2.0 - 1.0))
    * createRotationMatrixAxisAngle(
        vec3(1.0, 0.0, 0.0), 0.5 + 1.5 * (((NormalizedMouse.y == 0.0 ? 0.27 : NormalizedMouse.y) * 1.0) * 2.0 - 1.0))
    * proj;
}
